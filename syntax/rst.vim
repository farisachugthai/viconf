...
" My additions:
  " Odd to me that this wasn't present.

  " numbers
  syn match rstNumber	"\<\d\+\>"

  " Note that syn case ignore is set in $VIMRUNTIME/syntax/rst.vim
  " so fixme and todo should work. My additions are on the 2nd line
  syn keyword     rstTodo             contained FIXME TODO XXX NOTE
        \

" Original:
" Also modified the way that rstEmphasis is defined so that by default
" a lone * doesn't fucking highlight because ``*args and ``**kwargs`
" will end up destroying the output of
" `:r!pydoc foo | set syntax=rst`
" Which i run quite frequently
...
if exists('g:rst_syntax_enable_emphasis')
